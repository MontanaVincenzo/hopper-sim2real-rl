...
best_target_model = {
    ...
    "clip_range": 0.0,
    "entropy_coef": 0.0,
    "target_to_target_mean": 0.0,
    "target_to_target_std": 0.0,
    "#run": 10,
    "entropy_coefficient": 0.0,
    "source_to_target_std": 0.0
}
sweep_config_source = {
...
def sweep_train_source():
    ...
    wandb.init(project="custom-walker-ppo", dir="./source_logs_train/run_"+str(run_number))
    config = wandb.config
...
def sweep_train_target():
    ...
    wandb.init(project="custom-walker-ppo", dir="./target_logs_train/run_"+str(run_number))
    config = wandb.config
    print(config)
    # Initialize environments
    env_target = Monitor(gym.make("CustomWalker2d-target-v0"))
...
def main(args):
    ...
    source_model_path = "./models/source/run_3" # Ricorda di aggiungere il percorso che vuoi testare
    target_model_path = "./models/target/run_10" # Ricorda di aggiungere il percorso che vuoi testare
    os.makedirs(source_model_path, exist_ok=True)
    os.makedirs(target_model_path, exist_ok=True)
    ...
    if not args.test:
        ...
        # sweep_id_source = wandb.sweep(sweep_config_source, project="custom-walker-ppo")
        # wandb.agent(sweep_id_source, function=sweep_train_source, count=10)
        
        
        # Load config for target model, then train
        sweep_id_target = wandb.sweep(sweep_config_target, project="custom-walker-ppo")
        wandb.agent(sweep_id_target, function=sweep_train_target, count=5)
        ...
        env_id_source = "CustomWalker2d-source-v0"   # env_id name
        env_id_target = "CustomWalker2d-target-v0"   # env_id name
        ...
        if args.plot:
            ...
            np.save("../../Plots/walker/s2t_rewards.npy", episode_rewards)
            ...
            np.save("../../Plots/walker/t2t_rewards.npy", episode_rewards)
...
if __name__ == '__main__':
    ...
    #args = parse_args(['--timesteps', '1000000'])
