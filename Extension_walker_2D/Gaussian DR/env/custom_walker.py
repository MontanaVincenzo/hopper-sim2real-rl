...
class Walker2dEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        domain=None,  # New parameter to differentiate environments
        **kwargs
    ):
        ...
        # Modify torso mass if in the "target" domain
        if domain == "target":
            self.sim.model.body_mass[1] += 3.0
    ...
    def get_body_masses(self):
        """
        Retrieve the masses of torso, right leg, right foot, left leg, and left foot.
        """
        model = self.sim.model
        body_names = model.body_names

        # Find specific bodies
        torso_index = body_names.index('torso')
        ...
        # Retrieve the masses
        torso_mass = model.body_mass[torso_index]
        right_foot_mass = model.body_mass[right_foot_index]
        right_leg_mass = model.body_mass[right_leg_index]
        left_foot_mass = model.body_mass[left_foot_index]
        left_leg_mass = model.body_mass[left_leg_index]

        return {
            "torso_mass": torso_mass,
            "right_leg_mass": right_leg_mass,
            "right_foot_mass": right_foot_mass,
            "left_leg_mass": left_leg_mass,
            "left_foot_mass": left_foot_mass,
        }


    def set_random_parameters(self, right_thigh_mass, right_leg_mass, right_foot_mass, left_thigh_mass, left_leg_mass, left_foot_mass, delta_foot, delta_leg, delta_thigh, distribution_type='uniform'):
        """Set random masses"""

        self.set_parameters(self.sample_parameters(right_thigh_mass, right_leg_mass, right_foot_mass, left_thigh_mass, left_leg_mass, left_foot_mass, delta_foot, delta_leg, delta_thigh, equal_masses=True, distribution_type=distribution_type))

    def sample_parameters(self, right_thigh_mass, right_leg_mass, right_foot_mass, left_thigh_mass, left_leg_mass, left_foot_mass, delta_foot, delta_leg, delta_thigh, equal_masses=True, distribution_type='uniform'):
        """Sample masses according to a domain randomization distribution"""
        
        if equal_masses:
            if distribution_type == 'uniform':     
                thigh_mass_sample = np.random.uniform(right_thigh_mass-float(right_thigh_mass * delta_thigh), right_thigh_mass+float(right_thigh_mass * delta_thigh))
                right_thigh_mass_sample = thigh_mass_sample
                left_thigh_mass_sample = thigh_mass_sample

                leg_mass_sample = np.random.uniform(right_leg_mass-float(right_leg_mass * delta_leg), right_leg_mass+float(right_leg_mass * delta_leg))
                right_leg_mass_sample = leg_mass_sample
                left_leg_mass_sample = leg_mass_sample

                foot_mass_sample = np.random.uniform(right_foot_mass-float(right_foot_mass * delta_foot), right_foot_mass+float(right_foot_mass * delta_foot))
                right_foot_mass_sample = foot_mass_sample
                left_foot_mass_sample = foot_mass_sample

            elif distribution_type == 'gaussian':
                thigh_mass_sample = np.maximum(0, np.random.normal(right_thigh_mass, right_thigh_mass * delta_thigh))
                right_thigh_mass_sample = thigh_mass_sample
                left_thigh_mass_sample = thigh_mass_sample


                leg_mass_sample = np.maximum(0, np.random.normal(right_leg_mass, right_leg_mass * delta_leg))
                right_leg_mass_sample = leg_mass_sample
                left_leg_mass_sample = leg_mass_sample

                
                foot_mass_sample = np.maximum(0, np.random.normal(right_foot_mass, right_foot_mass * delta_foot))
                right_foot_mass_sample = foot_mass_sample
                left_foot_mass_sample = foot_mass_sample
            ...
            right_thigh_mass_sample = np.random.uniform(right_thigh_mass-float(right_thigh_mass * delta_thigh), right_thigh_mass+float(right_thigh_mass * delta_thigh)) 
            right_leg_mass_sample = np.random.uniform(right_leg_mass-float(right_leg_mass * delta_leg), right_leg_mass+float(right_leg_mass * delta_leg))
            right_foot_mass_sample = np.random.uniform(right_foot_mass-float(right_foot_mass * delta_foot), right_foot_mass+float(right_foot_mass * delta_foot))
            left_thigh_mass_sample = np.random.uniform(left_thigh_mass-float(left_thigh_mass * delta_thigh), left_thigh_mass+float(left_thigh_mass * delta_thigh)) 
            left_leg_mass_sample = np.random.uniform(left_leg_mass-float(left_leg_mass * delta_leg), left_leg_mass+float(left_leg_mass * delta_leg))
            left_foot_mass_sample = np.random.uniform(left_foot_mass-float(left_foot_mass * delta_foot), left_foot_mass+float(left_foot_mass * delta_foot))

        return [right_thigh_mass_sample, right_leg_mass_sample, right_foot_mass_sample, left_thigh_mass_sample, left_leg_mass_sample, left_foot_mass_sample]
    ...
    def set_parameters(self, task):
        ...
        self.sim.model.body_mass[right_thigh_index] = task[0]
        self.sim.model.body_mass[right_leg_index] = task[1]
        self.sim.model.body_mass[right_foot_index] = task[2]
        self.sim.model.body_mass[left_thigh_index] = task[3]
        self.sim.model.body_mass[left_leg_index] = task[4]
        self.sim.model.body_mass[left_foot_index] = task[5]
