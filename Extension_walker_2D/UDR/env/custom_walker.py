...
class Walker2dEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def set_random_parameters(self, right_thigh_mass, right_leg_mass, right_foot_mass, left_thigh_mass, left_leg_mass, left_foot_mass, delta_thigh, delta_leg, delta_foot, distribution_type='uniform'):
        """Set random masses"""

        self.set_parameters(self.sample_parameters(right_thigh_mass, right_leg_mass, right_foot_mass, left_thigh_mass, left_leg_mass, left_foot_mass, delta_thigh, delta_leg, delta_foot, equal_masses=True, distribution_type=distribution_type))

    def sample_parameters(self, right_thigh_mass, right_leg_mass, right_foot_mass, left_thigh_mass, left_leg_mass, left_foot_mass, delta_thigh, delta_leg, delta_foot, equal_masses=True, distribution_type='uniform'):
        ...
        if equal_masses:
            ...
            right_thigh_mass_sample = np.random.uniform(right_thigh_mass-float(right_thigh_mass * delta), right_thigh_mass+float(right_thigh_mass * delta)) 
            right_leg_mass_sample = np.random.uniform(right_leg_mass-float(right_leg_mass * delta), right_leg_mass+float(right_leg_mass * delta))
            right_foot_mass_sample = np.random.uniform(right_foot_mass-float(right_foot_mass * delta), right_foot_mass+float(right_foot_mass * delta))
            left_thigh_mass_sample = np.random.uniform(left_thigh_mass-float(left_thigh_mass * delta), left_thigh_mass+float(left_thigh_mass * delta)) 
            left_leg_mass_sample = np.random.uniform(left_leg_mass-float(left_leg_mass * delta), left_leg_mass+float(left_leg_mass * delta))
            left_foot_mass_sample = np.random.uniform(left_foot_mass-float(left_foot_mass * delta), left_foot_mass+float(left_foot_mass * delta))
